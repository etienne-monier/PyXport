...
def plot2im(mat, loc, reference=None, cmap='viridis'):
    ...
    Args:
        ...
        reference (numpy array): A reference matrix.
...
def save_dat(data, loc, sep=' '):
    ...
    Note:
        In the case of multiple arrays input, the arrays should have the save size. Otherwise, a ValueError will be raised.
    ...
    if type(data).__module__ == np.__name__:
        if data.ndim > 1:
            raise ValueError('The data was a numpy array of dimension {} where the only dimension allowed is 1. Give a tuple, list or dictionary instead.'.format(data.ndim))
    ...
    # Is it a list or tuple ?
        ...
        if len(set([type(a).__module__ == np.__name__ for a in data])) != 1:
            raise ValueError('Some elements of the data list were not numpy data.')
        if len(set([a.shape for a in data])) != 1:
            raise ValueError('Elements of the data list have inconsistent shapes.')
        # if len(data)>2:
        #   raise ValueError('A list/tuple of length superior to 2 was given. This is not accepted by pgfplotstable. Use a dictionary instead to add keywords.')
        data_out = np.asarray(data)

    # Is it a dico ?
    elif type(data) is dict:
        if len(set([type(a).__module__ == np.__name__ for a in list(data.values())])) != 1:
            raise ValueError('Some elements of the data list were not numpy data.')
    ...
    # Not known data
        ...
        raise ValueError('The data is not a numpy array, nor a tuple, nor a list, nor a dictionary. Instead, its type is {}'.format(type(data)))
